...
*/
public class CarolMSSQLSERVEREventFormatter implements IEventFormatter {   
    ...
    public byte[] formatEventBatch(List<SinkRecord> batch) throws IOException {        
        ...
        for(SinkRecord record : batch) {
            ...
            String aux = "Struct{after=Struct{";
            int start = recordValue.indexOf(aux);
            int end = recordValue.indexOf("}", start);
            String mystring = "{" + recordValue.substring(start+aux.length(), end) + "}";
            
            System.out.println(mystring.replaceAll("=", ":\"").replaceAll(",", "\",").replaceAll("}", "\"}"));

            events.add(mystring.replaceAll("=", ":\"").replaceAll(",", "\",").replaceAll("}", "\"}"));
